...
public class OpdBrowser extends ModalJFrame implements OpdEdit.SurgeryListener, OpdEditExtended.SurgeryListener {
	...
	private JPanel jSelectionWardPanel = null;
	...
	public JComboBox getWardBox() {
		if (jWardBox == null) {
			...
			jWardBox.setMaximumSize(new Dimension(300, 50));
	...
	class OpdBrowsingModel extends DefaultTableModel {
		...
		public OpdBrowsingModel(Ward ward, String diseaseTypeCode, String diseaseCode, LocalDate dateFrom, LocalDate dateTo, int ageFrom, int ageTo,
			...
			try {
				pSur = opdBrowserManager.getOpd(ward, diseaseTypeCode, diseaseCode, dateFrom, dateTo, ageFrom, ageTo, sex, newPatient, 0);
			} catch (OHServiceException ohServiceException) {
				MessageDialog.showExceptions(ohServiceException);
			}
		}
